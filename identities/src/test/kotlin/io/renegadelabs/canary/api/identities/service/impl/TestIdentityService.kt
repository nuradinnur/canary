            ...
            then("it should return empty") {
                ...
                StepVerifier.create(verification)
                    .consumeNextWith { identity ->
                        ...
                        identity.id.shouldNotBeNull().shouldBe(3)
                        ...
                        identity.expired.shouldNotBeNull().shouldBe(false)
                        identity.locked.shouldNotBeNull().shouldBe(false)
                        identity.credentialsExpired.shouldNotBeNull().shouldBe(false)
                        identity.enabled.shouldNotBeNull().shouldBe(true)
                    }
                    .verifyComplete()
                    .shouldBeLessThan(Duration.ofMillis(250))
            }
        ...
        `when`("I want to get its identity") {
            then("it should return the identity created") {
                ...
                StepVerifier.create(publisher)
                    .consumeNextWith { identity ->
                        identity.shouldNotBeNull()
                        identity.id.shouldNotBeNull().shouldBe(1)
        ...
        `when`("I want to get the associated identity") {
            then("it should return empty") {
                ...
                StepVerifier.create(publisher)
                    .consumeNextWith { identity ->
                        identity.shouldNotBeNull()
                        identity.id.shouldNotBeNull().shouldBe(0)
